...
#shared singleton
def self.shared
  @@shared = GameKernel.new unless @@shared
  return @@shared
end
...
def resume_game(new_action=nil)
	...
	until @state.game_winner(@state) do
		...
		if(!action) #player passed
			...
			if(@state.top_stack_item && @turn_order_delegate.is_action_settled(@state.active_action))
				@state.resolve_action
			elsif(!@state.active_action || @state.active_action && turn_order_delegate.is_action_settled(@state.active_action))
